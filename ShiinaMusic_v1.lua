...
local COBALT_INDEX = 1

-- ══════════════════════════════════════════════
--  HELPER: request universal (Delta / Synapse / otros)
-- ══════════════════════════════════════════════
local function httpRequest(opts)
    -- Delta usa http.request; Synapse usa syn.request; algunos usan request() global
    ...
    elseif HttpService and HttpService.RequestAsync then
        -- fallback nativo (solo funciona fuera de exploits, igual lo dejamos)
...
--  FIX: headers actualizados, fallback audioOnly,
--       uso de httpRequest universal
-- ══════════════════════════════════════════════
local function getCobaltAudio(videoId, callback)
    ...
    local function tryNext()
        ...
        -- Intentamos primero con downloadMode="audio" (API nueva)
        -- Si falla, reintentamos con "audioOnly" (API vieja) en el mismo host
        local function attempt(mode)
            local ok, res = pcall(function()
                return httpRequest({
                    ...
                    Headers = {
                        ...
                        -- algunas instancias requieren este header extra
                        ["x-api-key"]    = "",
                    },
            ...
            -- status puede ser "tunnel", "redirect", o simplemente tener .url
            if data.url then
                return data.url
            end
            return nil
        ...
        local url = attempt("audio") or attempt("audioOnly")
        if url then
            callback(url, nil)
        else
            tryNext()
...
-- ══════════════════════════════════════════════
--  YOUTUBE SEARCH
--  FIX: búsqueda robusta del itemSectionRenderer
--       en cualquier slot (no solo contents[1])
-- ══════════════════════════════════════════════
local function searchYouTube(query, callback)
    task.spawn(function()
        ...
        -- FIX: recorremos TODOS los slots de contents buscando itemSectionRenderer
        -- en lugar de asumir que siempre está en contents[1]
        local ok2 = pcall(function()
            ...
            for _, section in ipairs(primary) do
                -- puede estar directo o envuelto
                    ...
                    for _, item in ipairs(isr.contents) do
                        if item.videoRenderer then
                            ...
                            local title = v.title
                                and v.title.runs
                                and v.title.runs[1]
                                and v.title.runs[1].text
                                or "Sin titulo"
                            local ch = v.ownerText
                                and v.ownerText.runs
                                and v.ownerText.runs[1]
                                and v.ownerText.runs[1].text
                                or ""
                            local dur = v.lengthText
                                and v.lengthText.simpleText
                                or "?"
                            if id then
...
-- ══════════════════════════════════════════════
--  AUDIO PLAYBACK
--  FIX: uso de syn.play_audio si disponible (Delta),
--       con fallback a Sound + proxy para URLs externas
-- ══════════════════════════════════════════════
...
local function playSound(url, onEnded)
    ...
    -- FIX: Roblox no acepta URLs HTTP externas como SoundId directamente.
    -- Delta expone syn.play_audio que bypasea esa restricción.
    -- Si no está disponible, intentamos con el proxy rbxhttp que algunos exploits exponen,
    -- y como último recurso metemos la URL igual (puede funcionar en algunos exploits con hooks).
    ...
    if syn and syn.play_audio then
        -- Método preferido en Synapse/Delta: reproduce URL externa directamente
        syn.play_audio(s, url)
    else
        -- Intentar con rbxassetid proxy si el executor lo hookea,
        -- o directamente con la URL (funciona en Delta con http hook)
        s.SoundId = url
    end
